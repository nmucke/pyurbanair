#!/bin/bash
# Thin wrapper: DELFTBLUE sweep over the observation interval (esmda.interval_seconds,
# the temporal-aggregation bin width) for the pyudales backend.
#
...
#
#     bash job_scripts/delftblue/pyudales/sweep_interval_rollout_esmda_from_truth.sh
#
# Any extra arguments are forwarded as Hydra overrides to EVERY job, e.g.:
#
#     bash job_scripts/delftblue/pyudales/sweep_interval_rollout_esmda_from_truth.sh esmda.seed=1
set -euo pipefail
