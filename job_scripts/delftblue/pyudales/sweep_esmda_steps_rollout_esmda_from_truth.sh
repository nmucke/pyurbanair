#!/bin/bash
# Thin wrapper: DELFTBLUE sweep over ESMDA step count (iterations per window) for the pyudales backend.
#
...
#
#     bash job_scripts/delftblue/pyudales/sweep_esmda_steps_rollout_esmda_from_truth.sh
#
# Any extra arguments are forwarded as Hydra overrides to EVERY job, e.g.:
#
#     bash job_scripts/delftblue/pyudales/sweep_esmda_steps_rollout_esmda_from_truth.sh esmda.seed=1
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
exec bash "${SCRIPT_DIR}/../sweep_base.sh" steps "${SCRIPT_DIR}/rollout_esmda_from_truth.slurm" "$@"
