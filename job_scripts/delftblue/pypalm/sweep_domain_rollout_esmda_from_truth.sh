#!/bin/bash
# Thin wrapper: DELFTBLUE sweep over grid resolution (NX/NY/NZ) for the pypalm backend.
#
...
#
#     bash job_scripts/delftblue/pypalm/sweep_domain_rollout_esmda_from_truth.sh
#
# Any extra arguments are forwarded as Hydra overrides to EVERY job, e.g.:
#
#     bash job_scripts/delftblue/pypalm/sweep_domain_rollout_esmda_from_truth.sh esmda.seed=1
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
exec bash "${SCRIPT_DIR}/../sweep_base.sh" domain "${SCRIPT_DIR}/rollout_esmda_from_truth.slurm" "$@"
