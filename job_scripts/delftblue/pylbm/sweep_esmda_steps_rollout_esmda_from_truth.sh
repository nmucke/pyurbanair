#!/bin/bash
# Thin wrapper: DELFTBLUE sweep over ESMDA step count (iterations per window) for the pylbm backend.
#
...
#
#     bash job_scripts/delftblue/pylbm/sweep_esmda_steps_rollout_esmda_from_truth.sh
#
# Any extra arguments are forwarded as Hydra overrides to EVERY job, e.g.:
#
#     bash job_scripts/delftblue/pylbm/sweep_esmda_steps_rollout_esmda_from_truth.sh esmda.seed=1
set -euo pipefail
