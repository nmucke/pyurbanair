#!/bin/bash
# Thin wrapper: SNELLIUS sweep over grid resolution (NX/NY/NZ) for the pypalm backend.
#
...
#
#     bash job_scripts/snellius/pypalm/sweep_domain_rollout_esmda_from_truth.sh
#
# Any extra arguments are forwarded as Hydra overrides to EVERY job, e.g.:
#
#     bash job_scripts/snellius/pypalm/sweep_domain_rollout_esmda_from_truth.sh esmda.seed=1
set -euo pipefail
