#!/bin/bash
# Thin wrapper: SNELLIUS sweep over ensemble size for the pyudales backend.
#
...
#
#     bash job_scripts/snellius/pyudales/sweep_ensemble_rollout_esmda_from_truth.sh
#
# Any extra arguments are forwarded as Hydra overrides to EVERY job, e.g.:
#
#     bash job_scripts/snellius/pyudales/sweep_ensemble_rollout_esmda_from_truth.sh esmda.seed=1
set -euo pipefail
