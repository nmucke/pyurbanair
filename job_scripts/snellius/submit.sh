...
# xlarge=96), which sets the cores requested (one worker per member), rounded up
# to the partition's minimum billable share (16 on rome, 24 on genoa) and capped
# at a single node. The worker count is passed through as
# `ensemble.num_parallel_processes` and the ensemble size as
# `ensemble.ensemble_size`. The run otherwise uses the medium-sized defaults
# baked into conf/run_esmda.yaml; pass ensemble.ensemble_size=N (or any hydra
