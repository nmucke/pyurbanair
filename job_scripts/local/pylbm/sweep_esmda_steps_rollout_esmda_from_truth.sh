#!/bin/bash
# Thin wrapper: LOCAL sweep over ESMDA step count (iterations per window) for the pylbm backend.
#
...
# Run from anywhere:
#
#     bash job_scripts/local/pylbm/sweep_esmda_steps_rollout_esmda_from_truth.sh
#
# Any extra arguments are forwarded as Hydra overrides to EVERY run, e.g.:
#
#     bash job_scripts/local/pylbm/sweep_esmda_steps_rollout_esmda_from_truth.sh esmda.seed=1
set -euo pipefail
