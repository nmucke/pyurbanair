#!/bin/bash
# Thin wrapper: LOCAL sweep over ESMDA step count (iterations per window) for the pypalm backend.
#
...
#
#     bash job_scripts/local/pypalm/sweep_esmda_steps_rollout_esmda_from_truth.sh esmda.seed=1
set -euo pipefail
