...
# the time-aggregation bin width) for the pypalm backend.
#
...
# Run from anywhere:
#
#     bash job_scripts/local/pypalm/sweep_interval_rollout_esmda_from_truth.sh
#
# Any extra arguments are forwarded as Hydra overrides to EVERY run, e.g.:
#
#     bash job_scripts/local/pypalm/sweep_interval_rollout_esmda_from_truth.sh esmda.seed=1
set -euo pipefail
