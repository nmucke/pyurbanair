...
def test_run_rollout_esmda(
    truth_model: str,
    assim_model: str,
    tmp_path: pathlib.Path,
    compose_test_cfg,
) -> None:
    """Spin-up path: rollout ESMDA seeds its initial ensemble from a single
    warm-up call to the assim model and runs one assimilation window."""
    from scripts.run_rollout_esmda import run
    ...
    run(compose_test_cfg(overrides, config_name="run_rollout_esmda"))
