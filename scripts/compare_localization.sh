#!/usr/bin/env bash
#
# Run scripts/run_esmda.py three times — no localization, correlation-based, and
# distance-based — on an otherwise identical joint state + time-varying-parameter
# ESMDA config, so the three localization strategies can be compared. Each run
# writes its outputs (incl. run_summary.yaml) to its own .temp/loc_<mode> dir.
#
...
#
# Runs five modes: none, correlation, distance (small radius), distance (large
# radius) — so the effect of the distance localization radius is visible — plus
# a parameter-only baseline (the `dynamic` smoother: estimate the time-varying
# parameters only, no state estimation).
#
...
#   TRUTH_MODEL    solver that generates the truth: pylbm|pyudales|pypalm  (pyudales)
...
case "${SIZE}" in
  ...
  large)
    ...
    size_args=(
      ...
      'obs.x_points=[0.0,0.0,20.0,20.0]' 'obs.y_points=[10.0,30.0,10.0,30.0]'
      'obs.z_points=[10.0,10.0,10.0,10.0]'
      'obs.validation_x_points=[10.0,10.0]' 'obs.validation_y_points=[15.0,25.0]'
      'obs.validation_z_points=[10.0,10.0]'
    )
    ;;
  *)
...
# inline-truth pylbm run. Combined with "${size_args[@]}" (domain + sensors).
common=(
  ...
  run.truth_dir=null run.skip_viz=false
  obs.interval_seconds=10.0
...
# echo "== 1/5  none (global update)  [SIZE=${SIZE}] =="
# run_esmda "${common[@]}" "${model_args[@]}" "${size_args[@]}" \
#   esmda/localization=none \
#   hydra.run.dir=.temp/loc_none

echo "== 2/5  correlation-based localization (state only)  [SIZE=${SIZE}] =="
run_esmda "${common[@]}" "${model_args[@]}" "${size_args[@]}" \
...
echo "== 3/5  distance localization, small radius R=${RADIUS_SMALL} (state only)  [SIZE=${SIZE}] =="
run_esmda "${common[@]}" "${model_args[@]}" "${size_args[@]}" \
...
# echo "== 4/5  distance localization, large radius R=${RADIUS_LARGE} (state only)  [SIZE=${SIZE}] =="
# run_esmda "${common[@]}" "${model_args[@]}" "${size_args[@]}" \
#   esmda/localization=distance \
...
echo "== 5/5  parameter-only (dynamic smoother, no state estimation)  [SIZE=${SIZE}] =="
run_esmda "${common[@]}" "${model_args[@]}" "${size_args[@]}" \
  esmda/smoother=dynamic esmda/localization=none \
  hydra.run.dir=.temp/loc_param_only

echo
# Build a comparison table (mean metrics, <=4 decimals) across the five modes
# from each run's run_summary.yaml: the per-parameter RMSE/CRPS and the held-out
# validation-sensor RMSE/CRPS (run.skip_viz=false also leaves the figures in
# each .temp/loc_<mode>/ dir). param_only = the dynamic smoother (no state est.).
# modes = ["none", "correlation", "distance_small", "distance_large", "param_only"]
pixi run --environment dev python - <<'PY'
import os, yaml

modes = ["correlation", "distance_small", "param_only"]
summary = {}
for m in modes:
...
PY
echo
echo "Figures + full results per mode: .temp/loc_{none,correlation,distance_small,distance_large,param_only}/"
