...
def model_slice(run, time_s: float, z_m: float = PED_Z
...
# ---------------------------------------------------------------------------
def param_panel(ax, *, time, post_mean, post_std=None, prior_mean=None,
                prior_std=None, truth_t=None, truth=None, edges=None,
                extra: Sequence[dict] = (), shade=True, post_color=None,
                post_label="posterior"):
    """One parameter panel: truth + prior band + posterior band (+ extra series)."""
    if shade:
        S.shade_windows(ax, edges)
    if prior_mean is not None:
        S.band(ax, time, prior_mean, prior_std, S.COLORS["prior"],
               alpha=0.18, label="prior", lw=1.6, ls="--")
    for s in extra:
        S.band(ax, s["time"], s["mean"], s.get("std"), s["color"],
               alpha=s.get("alpha", 0.0), label=s.get("label"),
               lw=s.get("lw", 1.6), ls=s.get("ls", "-"))
    if post_mean is not None:
        S.band(ax, time, post_mean, post_std,
               post_color or S.COLORS["posterior"], alpha=0.25,
               label=post_label, lw=2.2)
    if truth is not None:
        ax.plot(truth_t if truth_t is not None else time, truth,
                color=S.COLORS["truth"], lw=2.0, label="truth", zorder=6)
    S.mark_windows(ax, edges, annotate=False)
    ax.margins(x=0.01)


def plot_param_trajectories(out_path, *, prior, post, true, edges,
                            extra_by_param=None, post_label="posterior",
                            title=None):
    """Stacked α / |U| panels for a single run (truth/prior/posterior)."""
    params = [p for p in dataio.PARAMS if p in post.data_vars]
    fig, axes = plt.subplots(len(params), 1, figsize=(8.2, 2.7 * len(params)),
    ...
    for ax, p in zip(axes, params):
        ...
        prm = (np.asarray(prior[p].transpose("ensemble", "time").values)
               if prior is not None and p in prior.data_vars else None)
        tr_t = np.asarray(true["time"].values) if "time" in true.dims else t
        param_panel(
            ax, time=t, post_mean=pm.mean(0), post_std=pm.std(0),
            ...
            edges=edges, extra=(extra_by_param or {}).get(p, ()),
...
# ---------------------------------------------------------------------------
def plot_lines_vs_time(out_path, series, *, edges=None, ylabel="", xlabel="Time [s]",
                       title=None):
    """series: list of dict(time, y, color, label, lw?, ls?)."""
    fig, ax = plt.subplots(figsize=(8.6, 4.0), constrained_layout=True)
    S.shade_windows(ax, edges)
    for s in series:
        ax.plot(s["time"], s["y"], color=s["color"], label=s.get("label"),
                lw=s.get("lw", 1.8), ls=s.get("ls", "-"))
...
# ---------------------------------------------------------------------------
def plot_vs_resolution(out_path, panels, *, xlabel=r"Grid spacing $\Delta x$ [m]",
                       truth_dx=None):
    ...
    fig, axes = plt.subplots(1, n, figsize=(5.2 * n, 4.0),
                             constrained_layout=True, squeeze=False)
    axes = axes[0]
    all_dx = sorted({v for panel in panels for dx, _ in panel["by_model"].values()
                     for v in dx})
    for ax, panel in zip(axes, panels):
        for model, (dx, y) in panel["by_model"].items():
            order = np.argsort(dx)
            ax.plot(np.asarray(dx)[order], np.asarray(y)[order],
                    marker=S.MODEL_MARKERS.get(model, "o"), ms=7,
                    color=S.MODEL_COLORS.get(model), label=S.MODEL_LABELS.get(model, model))
        if truth_dx is not None and truth_dx not in all_dx:
            ax.axvline(truth_dx, color=S.COLORS["truth"], ls=":", lw=1.2,
...
# ---------------------------------------------------------------------------
def plot_sensor_timeseries(out_path, *, time, truth_ts, series, sensor_xyz,
                           edges=None, title=None):
    ...
    """
    n = truth_ts.shape[0]
    fig, axes = plt.subplots(n, 1, figsize=(8.2, 2.4 * n),
                             constrained_layout=True, squeeze=False)
    axes = axes[:, 0]
    for i, ax in enumerate(axes):
        ...
        for s in series:
            std = s.get("std")
            S.band(ax, time, s["ts"][i], None if std is None else std[i],
                   s["color"], alpha=0.2, label=s.get("label"), lw=1.8,
                   ls=s.get("ls", "-"))
        ax.plot(time, truth_ts[i], color=S.COLORS["truth"], lw=2.0,
...
# ---------------------------------------------------------------------------
def plot_field_error_grid(out_path, *, columns, truth_field, extent,
                          z_label="", mask2d=None, assim_xy=None, val_xy=None,
                          dpi=300):
    ...
    """
    ncol = len(columns)
    fig, axes = plt.subplots(2, ncol, figsize=(2.7 * ncol, 5.6),
                             constrained_layout=True, squeeze=False)
    ...
    errs = [None if lbl_is_truth else (_mask(f) - tf)
            for (lbl_is_truth, f) in
            [(i == 0, f) for i, (_, f) in enumerate(columns)]]
    emax = np.nanmax([np.nanmax(np.abs(e)) for e in errs if e is not None] or [1.0])

    im0 = im1 = None
    for j, (label, _) in enumerate(columns):
        ax = axes[0, j]
        im0 = ax.imshow(fields[j], origin="lower", extent=extent, aspect="equal",
                        cmap=S.CMAP_FIELD, vmin=vmin, vmax=vmax)
        ax.set_title(label, fontsize=10)
        _hatch(ax, mask2d, extent)
        if j == 0:
            if assim_xy is not None:
                ax.plot(assim_xy[:, 0], assim_xy[:, 1], "o", mfc="none",
                        mec="w", ms=5, mew=1.2, label="assim")
            if val_xy is not None:
                ax.plot(val_xy[:, 0], val_xy[:, 1], "^", mfc="none",
                        mec="w", ms=6, mew=1.2, label="valid")
        ax.set_xlabel("x [m]")
        if j == 0:
            ax.set_ylabel("y [m]")
        ...
        if errs[j] is None:
            ...
            im1 = axe.imshow(errs[j], origin="lower", extent=extent, aspect="equal",
                             cmap=S.CMAP_DIFF, vmin=-emax, vmax=emax)
...
def plot_field_time_montage(out_path, *, times, truth_fields, est_fields,
                            extent, est_label="estimate", z_label="",
                            mask2d=None, assim_xy=None, val_xy=None, dpi=300):
    ...
    fig, axes = plt.subplots(3, ncol, figsize=(2.7 * ncol, 7.8),
                             constrained_layout=True, squeeze=False)
    row_data = [("truth", tr, S.CMAP_FIELD, dict(vmin=0, vmax=vmax)),
    ...
    for ri, (rlabel, data, cmap, kw) in enumerate(row_data):
        for j, t in enumerate(times):
            ax = axes[ri, j]
            ims[ri] = ax.imshow(data[j], origin="lower", extent=extent,
    ...
    fig.colorbar(ims[2], ax=axes[2, :].ravel().tolist(), shrink=0.8, label="error [m/s]")
    return S.save_png(fig, out_path, dpi=dpi)
...
def _hatch(ax, mask2d, extent):
    ...
    if mask2d is None:
        return
    ax.contourf(np.linspace(extent[0], extent[1], mask2d.shape[1]),
                np.linspace(extent[2], extent[3], mask2d.shape[0]),
                mask2d.astype(float), levels=[0.5, 1.5], colors="none",
                hatches=["////"])
    ax.contour(np.linspace(extent[0], extent[1], mask2d.shape[1]),
               np.linspace(extent[2], extent[3], mask2d.shape[0]),
               mask2d.astype(float), levels=[0.5], colors="0.3", linewidths=0.5)
