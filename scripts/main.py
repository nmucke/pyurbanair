...
ANGLE_RANGE = np.random.randn(100) * 8
TRUE_ANGLE = 10.0
...
def esmda_step(
    ...
    N_e = len(angles)  # Number of ensemble members
    ...
    angles_dev = angles - angles_mean  # Shape: [N_e]
    ...
    C_MD = (1.0 / (N_e - 1)) * np.dot(angles_dev, pred_obs_dev)  # Shape: [N_d]
    ...
    angles_updated = np.zeros_like(angles)
    ...
    for j in range(N_e):
        ...
        angles_updated[j] = angles[j] + np.dot(C_MD, x)

    return angles_updated
...
def main() -> None:
    ...
    for i in range(NUM_ESMDA_STEPS):
        states = simulate_ensemble(forward_model, angles)
        pred_obs = [observation_operator(state) for state in states]
        pred_obs = np.array(pred_obs)
        angles = esmda_step(angles, true_obs, pred_obs, ALPHA, C_D)
        angles_history.append(angles)

        ensemble_mean_field = get_ensemble_mean_field(states)
    ...
    for i in range(NUM_ESMDA_STEPS + 1):
        ...
        axes[i, 3].hist(angles_history[i], **hist_args(i))
