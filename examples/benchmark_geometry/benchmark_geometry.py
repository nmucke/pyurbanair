...
class XieCastroBenchmarkGeometry:
    ...
    def __init__(self, 
                 heights = np.array([[10,  13.6,10, 2.8], 
                                     [13.6,10,  6.4,10], 
                                     [13.6,6.4,17.2,10], 
                                     [10,  6.4,13.6,10]]), 
        ...
        assert np.asarray(heights).shape == (4,4), "The Xie Castro Benchmark needs 4x4 building heights"
    ...
    def _create_building_list(self)->list[Building]:
        ...
        row_origins = np.array([[   tile_size/2,  tile_size/2],
        ...
        for origin, hs in zip(row_origins[idx,:],self.heights[idx,:]):
            for  i,h in enumerate(hs):
                buildings.append(Building(np.array([0, 2*i * tile_size]) + origin, size = [tile_size,tile_size], height=h))
        ...
        for origin, hs in zip(row_origins[idx,:], self.heights[idx,:]):
            ...
            print(hs[3])
            print(tile_size)
            buildings.append(Building(np.array([0, -3*tile_size/2]) + origin, size = [tile_size,tile_size/2], height=hs[3]))
            buildings.append(Building(np.array([0, 6*tile_size]) + origin, size = [tile_size, tile_size/2], height=hs[3]))
        return buildings
    ...
    def _to_uniform_cartesian(self)->tuple[np.array, np.array]:
        ...
        mask = np.zeros(shape=(self.num_cells[0], self.num_cells[1], self.num_cells[2]),dtype=bool)
        indices = []
    ...
    def to_palm(self)->str:
        pass


def test_BenchmarkGeometry(): 
...
def main(output_type: str, filename: str, resolution: int = 1):
    """
