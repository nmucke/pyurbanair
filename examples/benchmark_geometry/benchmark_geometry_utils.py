...
class Box(NamedTuple):
    ...
    def __call__(self, grid: Array):
        mask = (self.lower[:,None,None,None] < grid[:,:-1,:-1,:-1]) | np.isclose(self.lower[:,None,None,None],grid[:,:-1,:-1,:-1])
        mask &= (grid[:,1:,1:,1:] < self.upper[:,None,None,None]) | np.isclose(grid[:,1:,1:,1:] , self.upper[:,None,None,None])
        return mask
...
class Building:
    def __init__(self, lower: npt.ArrayLike, size: npt.ArrayLike, height: float, id: int=0):
        """
        ...
        self.coordinates = np.array([[lower[0], lower[1]],
                                    [upper[0], lower[1]],
                                    [upper[0], upper[1]],
                                    [lower[0], upper[1]]])
        self.height = height
    ...
    def _to_triangles(self)->np.array:
        vertices  = np.concatenate((np.column_stack((self.coordinates, np.full(len(self.coordinates), 0))), 
                                    np.column_stack((self.coordinates, np.full(len(self.coordinates), self.height)))))
        triangles = []
        normals = []
        # lower normal to x 
    ...
    def compute_index(self,x_coords,y_coords,z_coords):
        """
...
def display_building_list(buildings: list[Building], plot_transposed:bool=True):
...
def test_building_triangles():
    building = Building(np.array([0,0]),np.array([1,2]),3)