...
class ForwardModel(BaseForwardModel):
    ...
    def __init__(
        ...
        Args:
            ...
            warm_start: If True, the forward model will start from a warm start file.
        """
        super().__init__(results_dir=results_dir)

        self.inflow_angle = None
        self.velocity_magnitude = None
        self.pressure_gradient_magnitude = None

        self.from_warm_start = False
        ...
        self.save_warm_start = save_warm_start
        # Create a directory to store warm start files if requested
        if save_warm_start:
            warm_start_dir = pathlib.Path(
                f"{self.cwd}/.temp/warm_starts/{self.experiment_name}"
            )
            self.warm_start_dir = create_dir(warm_start_dir)
        else:
        ...
        move_files_to_dir(self.experiment_dir, self.temp_dir)
        ...
        if self.save_warm_start:
            self._apply_save_warm_start()

    def _move_warm_start_file(self) -> None:
        """Move the warm start files to the warm start directory.

        If the model has already started from a warm start file, there will be two sets
        of warm start files. Only moves the files with the largest timestep number
        (the newest ones generated by the current run).
        """
        if self.warm_start_dir is None:
            logger.warning("warm_start_dir is None, cannot move warm start files")
            return

        work_experiment_dir = self.work_dir.joinpath(self.experiment_name)
        if not work_experiment_dir.exists():
            logger.warning(
                f"Work experiment directory {work_experiment_dir} does not exist"
            )
            return

        # Find all files starting with "initd" in the work experiment directory
        initd_files = [
            item
            for item in work_experiment_dir.iterdir()
            if item.is_file() and item.name.startswith("initd")
        ]

        if not initd_files:
            logger.warning(f"No initd files found in {work_experiment_dir}")
            return

        # Extract timestep numbers from filenames
        # Format: initd{number}_{xxx}_{xxx}.{experiment_name}
        # Example: initd00000220_000_000.300
        file_timesteps = {}
        for initd_file in initd_files:
            # Extract the number after "initd" using regex
            match = re.match(r"initd(\d+)", initd_file.name)
            if match:
                timestep = int(match.group(1))
                if timestep not in file_timesteps:
                    file_timesteps[timestep] = []
                file_timesteps[timestep].append(initd_file)
            else:
                logger.warning(
                    f"Could not extract timestep from filename: {initd_file.name}"
                )

        if not file_timesteps:
            logger.warning(f"Could not extract timestep numbers from initd files")
            return

        # Find the maximum timestep number (newest files)
        max_timestep = max(file_timesteps.keys())
        logger.info(
            f"Found timesteps: {sorted(file_timesteps.keys())}, moving files with timestep {max_timestep}"
        )

        # Move only the files with the maximum timestep number
        for initd_file in file_timesteps[max_timestep]:
            destination = self.warm_start_dir / initd_file.name
            shutil.move(str(initd_file), str(destination))
            logger.info(
                f"Moved warm start file {initd_file.name} to {self.warm_start_dir}"
            )

    def _apply_save_warm_start(self) -> None:
        """Apply the save warm start flag."""
        # Set trestart = runtime in namoptions.{self.experiment_name}
        namoptions_path = (
            ...
            for line in f:
                ...
                if stripped.startswith("&RUN"):
                    ...
                    try:
                        ...
                        pass
                lines.append(line)
        # If we found the runtime_value, rewrite the file with trestart set to runtime under &RUN
        if runtime_value is not None:
            output_lines = []
            in_run = False
            trestart_replaced = False
            for line in lines:
                stripped = line.strip()
                if stripped.startswith("&RUN"):
                    in_run = True
                    ...
                    if not trestart_replaced:
                        output_lines.append(f"trestart     = {runtime_value}\n")
                        trestart_replaced = True
                    output_lines.append(line)
                    in_run = False
                elif in_run and "trestart" in stripped and "=" in stripped:
                    if not trestart_replaced:
                        output_lines.append(f"trestart     = {runtime_value}\n")
                        trestart_replaced = True
                    # Skip the original trestart line
                else:
                    output_lines.append(line)
            with open(namoptions_path, "w") as f:
                f.writelines(output_lines)

    def _apply_from_warm_start(self) -> None:
        """Apply the from warm start flag."""
        if self.warm_start_dir is None:
            logger.warning("warm_start_dir is None, cannot apply from warm start")
            return

        # Find all files starting with "initd" in the warm start directory
        initd_files = [
            item
            for item in self.warm_start_dir.iterdir()
            if item.is_file() and item.name.startswith("initd")
        ]

        if not initd_files:
            logger.warning(f"No initd files found in {self.warm_start_dir}")
            return

        # Get the first file (sorted alphabetically) and extract the prefix
        # Format: initd{number}_{xxx}_{xxx}.{experiment_name}
        first_file = sorted(initd_files)[0]
        file_name = first_file.name

        # Extract the prefix part (e.g., "initd00000220")
        # The format is: initd{number}_{xxx}_{xxx}.{experiment_name}
        # We want to extract everything up to the first underscore after the number
        parts = file_name.split("_")
        if len(parts) >= 1:
            # Extract "initd{number}" part
            initd_prefix = parts[0]
        else:
            logger.warning(f"Could not parse initd file name: {file_name}")
            return

        # Update namoptions file
        namoptions_path = (
            pathlib.Path(self.temp_dir) / f"namoptions.{self.experiment_name}"
        )
        ...
        if not namoptions_path.exists():
            logger.warning(
                f"namoptions.{self.experiment_name} not found, cannot apply from warm start"
            )
            return
        ...
        # Process the file to update lwarmstart and startfile in the &RUN section
        output_lines = []
        in_run = False
        lwarmstart_updated = False
        startfile_updated = False
        ...
        for line in lines:
            ...
            if stripped.startswith("&RUN"):
                ...
                # We're leaving the &RUN section, add any missing values before the closing /
                if not lwarmstart_updated:
                    output_lines.append("lwarmstart   = .true.\n")
                    lwarmstart_updated = True
                if not startfile_updated:
                    startfile_value = f"'{initd_prefix}_xxx_xxx.{self.experiment_name}'"
                    output_lines.append(f"startfile    = {startfile_value}\n")
                    startfile_updated = True
                output_lines.append(line)
                in_run = False
            elif in_run:
                # We're in the &RUN section, check if this line needs to be updated
                if "lwarmstart" in stripped and "=" in stripped:
                    output_lines.append("lwarmstart   = .true.\n")
                    lwarmstart_updated = True
                elif "startfile" in stripped and "=" in stripped:
                    startfile_value = f"'{initd_prefix}_xxx_xxx.{self.experiment_name}'"
                    output_lines.append(f"startfile    = {startfile_value}\n")
                    startfile_updated = True
                else:
                ...
                # Not in &RUN section, keep the line as is
                output_lines.append(line)
        ...
        logger.info(
            f"Updated namoptions.{self.experiment_name} with lwarmstart=.true. and "
            f"startfile='{initd_prefix}_xxx_xxx.{self.experiment_name}'"
        )
    ...
    def change_dirs(
        ...
        old_warm_start_dir = self.warm_start_dir

        # Create new directories
        new_temp_dir = create_dir(temp_dir)
        new_output_dir = create_dir(output_dir)
        if warm_start_dir is not None:
            new_warm_start_dir = create_dir(warm_start_dir)
        else:
            new_warm_start_dir = None
        ...
        self.experiment_name = experiment_name
        self.warm_start_dir = new_warm_start_dir
    ...
    def _apply_output_frequency(self) -> None:
        ...
        logger.info(
            f"Updated tfielddump to {self.output_frequency} in namoptions.{self.experiment_name}"
        )
    ...
    def _clean_warm_start_dir(self) -> None:
        """Clean the warm start directory by deleting all its files."""
        if self.warm_start_dir is None or not self.warm_start_dir.exists():
            return

        for f in self.warm_start_dir.iterdir():
            if f.is_file():
                try:
                    f.unlink()
    ...
    def run_single(
        ...
        if self.from_warm_start:
            move_files_to_dir(self.warm_start_dir, self.temp_dir)
            self._apply_from_warm_start()
        ...
        if self.save_warm_start:
            self._clean_warm_start_dir()
            self._move_warm_start_file()
