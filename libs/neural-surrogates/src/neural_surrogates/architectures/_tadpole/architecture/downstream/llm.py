...
# builds the subnetwork with ``attention_method="naive"`` (see
# ``neural_surrogates.architectures.tadpole_stepper``). Keep both imports
# optional so this module imports cleanly without Triton/mamba; a genuine
# hyper/mamba request then fails loud below with an actionable message.
try:  # pragma: no cover - Triton is intentionally not a dependency here
    from .hyper_attn.hyper_attn import HyperAttention
except ImportError:  # pragma: no cover
    HyperAttention = None
try:  # pragma: no cover - mamba_ssm is intentionally not a dependency here
    from .mamba import create_mamba_block
except ImportError:  # pragma: no cover
    create_mamba_block = None
...
class HyperAttentionBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        if HyperAttention is None:  # pragma: no cover - Triton not installed
            raise ImportError(
                "attention_method='hyper' requires HyperAttention, which needs a "
                "Triton flash-attention kernel that is not installed. Use "
                "attention_method='naive' instead."
            )
...
class SequentialModel(nn.Module):
    ...
    def __init__(
        ...
        if attention_method == "mamba":
            if create_mamba_block is None:  # pragma: no cover - mamba_ssm absent
                raise ImportError(
                    "attention_method='mamba' requires 'mamba_ssm', which is not "
                    "installed. Use attention_method='naive' instead."
                )
            ssm_cfg = {"d_state": 16}
